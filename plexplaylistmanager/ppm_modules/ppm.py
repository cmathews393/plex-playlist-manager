...
def sonarr_tag_sync(users: List[str]) -> Dict[str, List[str]]:
    ...
    user_shows = {user: [] for user in plex_users}  # Prepare a dictionary to store shows per user
    ...
    for key, show_ids in show_dict.items():
        ...
        if user_key in plex_users:
            for show_id in show_ids:
                ...
                user_shows[user_key].append(show_title)  # Append show title to the appropriate user
